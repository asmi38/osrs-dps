...
class Calcs extends Component {
  ...
  render(){
    ...
    return(
      <div className='calcs'>
        <Card size='small' style={{width: 315}}>
          <Enemy />
          <table className='calcs-table'>
            ...
            <tbody>
              ...
              <tr>
                <td><b>DPS:</b></td>
                <td className={dpsA > dpsB ? 'text-higher' : 'text-lower'}> {dpsA.toFixed(3)} </td>
                <td className={dpsB > dpsA ? 'text-higher' : 'text-lower'}> {dpsB.toFixed(3)} </td>
                <td> {dpsDiff.toFixed(2)}% </td>
              </tr>
              <tr>
                <td><b>Overkill DPS:</b></td>
                <td className={okA > okB ? 'text-higher' : 'text-lower'}> {okA.toFixed(3)} </td>
                <td className={okB > okA ? 'text-higher' : 'text-lower'}> {okB.toFixed(3)} </td>
                <td> {okDiff.toFixed(2)}% </td>
              </tr>
              <tr>
                <td><b>Time to kill:</b></td>
